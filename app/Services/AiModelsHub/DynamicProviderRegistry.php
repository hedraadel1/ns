...
class DynamicProviderRegistry
{
    ...
    public function syncModels($providerId)
    {
        ...
        try {
            ...
            $response = Http::get($provider->models_fetch_endpoint);

            if (!$response->successful()) {
                throw new \Exception("Failed to fetch models from provider");
            }

            $modelsData = $response->json();

            // Process and store models
            $this->processModels($provider->id, $modelsData);
            ...
            // Clear model caches
            $this->clearModelCaches($provider->id);
