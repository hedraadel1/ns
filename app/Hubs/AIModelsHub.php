...
class AIModelsHub
{
    ...
    protected function getProviderService(string $providerId, string $providerName, string $apiKey): ?AiProviderInterface
    {
        ...
        // Create new provider instance based on provider name
        switch (strtolower($providerName)) {
            case 'openai':
                $this->providers[$providerId] = new \App\Services\AiModelsHub\OpenAIProvider($providerId, $this->encryptedKeyStorage);
                break;
            case 'groq':
                $this->providers[$providerId] = new \App\Services\AiModelsHub\GroqProvider($providerId, $this->encryptedKeyStorage);
                break;
            case 'anthropic':
                $this->providers[$providerId] = new \App\Services\AiModelsHub\AnthropicProvider($providerId, $this->encryptedKeyStorage);
                break;
            case 'google':
            case 'gemini':
                $this->providers[$providerId] = new \App\Services\AiModelsHub\GoogleGeminiProvider($providerId, $this->encryptedKeyStorage);
                break;
            default:
                Log::warning("Unknown provider type: {$providerName}");
                return null;
        }
