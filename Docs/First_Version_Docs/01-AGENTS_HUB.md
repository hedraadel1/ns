...
Security & Isolation
- Agents execute in restricted environments; policies limit accessible tools and data scopes per agent.
- Service-to-service calls authenticated via short-lived tokens; tool credentials stored in secrets manager.
- Audit every agent action with `trace_id` and persist logs to `agent_runtime_logs` and `audit_trails`.
