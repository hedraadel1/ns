...
- Security: TLS everywhere, service tokens, request signing for webhooks, data encryption at rest.
...
Dependencies:
- MySQL (core schema), Redis, Pinecone, S3-compatible storage, external AI providers, message broker (Kafka/RabbitMQ optional).

Next steps:
- Finalize hub contracts in `02-HUB_ARCHITECTURE.md` and define API/JSON schemas in `05-API_DESIGN.md`.
